...
export const RepoSidebar = memo(function RepoSidebar({
  ...
  const showPlaceholder = typeof placeholderMessage === "string" && placeholderMessage.length > 0;
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#1E1E35] bg-[#0E0E1A]/95 px-4 py-3 backdrop-blur">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            disabled={isBusy || showPlaceholder}
            onClick={onRefresh}
            title="Refresh repos"
            type="button"
          >
            <svg
              className={isLoading && !showPlaceholder ? "animate-spin" : ""}
              width="14"
              height="14"
              viewBox="0 0 24 24"
              fill="none"
      ...
      <div className="flex-1 overflow-y-auto">
        {showPlaceholder ? (
          <div className="flex h-full min-h-[280px] flex-col items-center justify-center gap-3 px-6 py-12 text-center">
            ...
            <p className="max-w-[16rem] text-sm text-[#9B9BB8]">
              {placeholderMessage}
            </p>
          </div>
        ) : isLoading ? (
      ...
      {/* Footer count */}
      {!showPlaceholder && !isLoading && !error && repos.length > 0 && (
